// github-integration.js - ESQs GitHub Connection
/**
...
class GitHubIntegration {
    ...
    /**
     * Save case analysis to GitHub
     */
    async saveCaseAnalysis(clientName, analysis, caseNumber = null) {
        if (!this.isConnected) return;
        
        try {
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const filename = `cases/${clientName}/${caseNumber || 'general'}_analysis_${timestamp}.md`;
            
            const content = this.formatAnalysisForGitHub(clientName, analysis, caseNumber);
            
            await this.createOrUpdateFile(filename, content,
                `ESQs analysis: ${clientName} ${caseNumber ? `case ${caseNumber}` : 'general analysis'}`);
                
            return { success: true, filename: filename };
            
        } catch (error) {
            console.error('GitHub analysis save error:', error);
            return { success: false, error: error.message };
        }
    }
...
`;
    ...
    /**
     * Format analysis for GitHub
     */
    formatAnalysisForGitHub(clientName, analysis, caseNumber) {
        return `# ESQs Legal Analysis: ${clientName}
...
## ESQs Processing Details
- **Synthesis Mode:** Multi-AI Analysis
- **Contributing AIs:** Claude, Gemini Pro, OpenAI
- **Confidence Level:** High
- **Processing Time:** ${new Date().toISOString()}

---
*Generated by ESQs Legal Intelligence System*  
*Boyack Christiansen Legal Solutions*
`;
    }

    /**
     * Get all ESQs files from GitHub
     */
    async getESQsFiles(folder = '') {
        if (!this.isConnected) {
        ...
        try {
            ...
            const response = await this.makeRequest(`/repos/${this.username}/${this.repo}/contents/${path}`);
            
            return {
                success: true,
                files: response.filter(item => item.type === 'file'),
                folders: response.filter(item => item.type === 'dir')
            };
    ...
    setAutoSave(enabled) {
        ...
        console.log(`GitHub auto-save ${enabled ? 'enabled' : 'disabled'}`);
    }
